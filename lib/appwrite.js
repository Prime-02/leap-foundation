...
export const fetchAllFiles = async () => {
  try {
    // Fetch all files in a specific bucket (replace with your bucket ID)
    const bucketId = storageId;
    const response = await storage.listFiles(bucketId);
    const filesWithUrls = response.files.map((file) => {
      const fileId = file.$id;

      // Use `getFileView` for a viewable URL or `getFileDownload` for a downloadable link
      const fileUrl = storage.getFileView(bucketId, fileId);

      return {
        ...file, // Include the original file metadata
